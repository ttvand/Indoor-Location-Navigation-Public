...
# 26 probed ids with each submission
offset_26 = 0
holdout_fn_ids = np.arange(106 + 26 * offset_26, 106 + 26 * (offset_26 + 1))
num_test_public = 1527
target_unit_change = -0.01

data_folder = utils.get_data_folder()
submission_folder = data_folder / 'submissions'
max_submission_path = submission_folder / 'probe_max_submission.csv'
submission_path = submission_folder / ('holdout_' + str(int(
    holdout_fn_ids[0])) + '_' + str(int(holdout_fn_ids[-1])) + '.csv')
