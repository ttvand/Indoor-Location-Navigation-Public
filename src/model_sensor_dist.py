...
def loss(data, preds):
  return torch.sqrt(((data['dist'] - preds)**2).mean())


def metric(targets, preds):
  return np.sqrt(((targets - preds)**2).mean())
...
class ZNN(torch.nn.Module):
  ...
  def forward(self, batch):
    ...
    return torch.sqrt((out.sum(1)**2).sum(-1))
...
class TrainingLoop():
  ...
  def run(self):
    for epoch in range(self.config.get('n_epochs')):
      ...
      print(f" {epoch:3d}:   " + f"metric {res['metric']:7.4f}   " +
    ...
    if self.valid_loader is not None:
      ...
      with torch.no_grad():
        for i, data in enumerate(tqdm(self.valid_loader)):
          ...
          all_targets.append(data['dist'].detach().cpu().numpy())
...
def run(mode, fast=False):
    print(f"Generating sensor distance predictions in mode {mode}")
    data_folder = utils.get_data_folder()
    if mode == 'cv':
      results_path = data_folder.parent / 'Models' / 'sensor_distance' / mode
    else:
      results_path = data_folder.parent / 'Models' / 'sensor_distance' / (
        'predictions')
